...
class ZerocoinSpendTest(CircuitTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zcrct_bal, crct_bal):
            zcrct_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zcrct_bal)
            crct_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], crct_bal)
            return zcrct_bal, crct_bal
        ...
        zcrct_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zcrct_balance, 6666)
        ...
        zcrct_balance, balance = check_balances(denom_0, zcrct_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zcrct_balance, balance = check_balances(denom_2, zcrct_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zcrct_balance, balance = check_balances(denom_3, zcrct_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zcrct",
        ...
        _, _ = check_balances(denom_1, zcrct_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
