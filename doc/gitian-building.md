...
*Setup instructions for a gitian build of CIRCUIT Core using a VM or physical system.*
...
to the CIRCUIT GitHub release page.
...
- [Building CIRCUIT Core](#building-circuit-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/circuit-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/circuit-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/circuit-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/circuit-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/circuit-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/circuit-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/circuit-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@circuit.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/circuit-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/circuit-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/circuit-osx-*-build.assert
```
