                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zCRCT ought to be enough for anybody." - Bill Gates, 2017
    ui->zCRCTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCRCTSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCRCTpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzCRCT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRCT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zCRCT, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCRCT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCRCT in ") +
...
void PrivacyDialog::on_pushButtonSpendzCRCT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRCT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zCRCT, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zCRCT
        sendzCRCT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCRCT
    sendzCRCT();
}

void PrivacyDialog::on_pushButtonZCrctControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZCrctControlDialog* zCrctControl = new ZCrctControlDialog(this);
    zCrctControl->setModel(walletModel);
    zCrctControl->exec();
}

void PrivacyDialog::setZCrctControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCrctSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCRCT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Circuit Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCRCTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCRCT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CRCT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCRCT</b>";
    ...
    // use mints from zCRCT selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCrctControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCrctControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCRCT transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zCRCT was spent successfully update the addressbook with the label
    ...
    // Clear zcrct selector in case it was used
    ZCrctControlDialog::setSelectedMints.clear();
    ui->labelzCrctSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCRCT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Crct, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zCRCT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zcrctTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCRCT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCRCT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCRCT "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zCRCT "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zCRCT </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zCRCT </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zCRCT </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzCRCTSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzCRCT->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zCRCT
        //ui->pushButtonMintzCRCT->setEnabled(false);
        //ui->pushButtonMintzCRCT->setToolTip(tr("zCRCT is currently disabled due to maintenance."));

        // Spend zCRCT
        ui->pushButtonSpendzCRCT->setEnabled(false);
        ui->pushButtonSpendzCRCT->setToolTip(tr("zCRCT is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zCRCT
        //ui->pushButtonMintzCRCT->setEnabled(true);
        //ui->pushButtonMintzCRCT->setToolTip(tr("PrivacyDialog", "Enter an amount of CRCT to convert to zCRCT", 0));

        // Spend zCRCT
        ui->pushButtonSpendzCRCT->setEnabled(true);
        ui->pushButtonSpendzCRCT->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
