    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("CRCT");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zCRCT");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (CRCT) or private coins (zCRCT)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a CIRCUIT address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 CRCT");
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send CRCT");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zCRCT");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZcrct = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcrct));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZcrct ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendCrct = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendCrct ? AskPassphraseDialog::Context::Send_CRCT : AskPassphraseDialog::Context::Send_zCRCT, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendCrct) ? send(recipients) : sendZcrct(recipients)) {
...
bool SendWidget::sendZcrct(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zCRCT is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZcrct(
        ...
        inform(tr("zCRCT transaction sent!"));
        ZCrctControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZCRCT_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zCRCT require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zCRCT transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isCRCT){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zCrctControl->exec();
            ui->btnCoinControl->setActive(!ZCrctControlDialog::setSelectedMints.empty());
            zCrctControl->deleteLater();
        } else {
            inform(tr("You don't have any zCRCT in your balance to select."));
        }
    }
}
...
void SendWidget::onCRCTSelected(bool _isCRCT){
    isCRCT = _isCRCT;
    setCssProperty(coinIcon, _isCRCT ? "coin-icon-crct" : "coin-icon-zcrct");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress crctAdd = CBitcoinAddress(address.toStdString());
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(crctAdd.Get(), dialog->getLabel().toStdString(), "send")) {
