...
#include "qt/circuit/privacywidget.h"
#include "qt/circuit/forms/ui_privacywidget.h"
#include "qt/circuit/qtutils.h"
#include "guiutil.h"
#include "qt/circuit/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(CIRCUITGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zCRCT anonymizes your CRCT by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zCRCT or convert back to CRCT"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 CRCT ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zCRCT");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zCRCT");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zCRCT");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zCRCT");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zCRCT");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zCRCT");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zCRCT");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zCRCT");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zCRCT Movements"));
    ...
    // Only Convert to CRCT enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzCRCT->setTitleClassAndText("btn-title-grey", "Total 0 zCRCT");
    ui->btnTotalzCRCT->setSubTitleClassAndText("text-subtitle", "Show denominations of zCRCT owned.");
    ui->btnTotalzCRCT->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select CRCT outputs to mint into zCRCT.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zCRCT");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzCRCT, SIGNAL(clicked()), this, SLOT(onTotalZcrctClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zCRCT");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of CRCT to mint into zCRCT"));
    }else{
        btnText = tr("Convert back to CRCT");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zCRCT to convert back into CRCT"));
    }
...
void PrivacyWidget::onTotalZcrctClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzCRCT->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzCRCT->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zCRCT is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zCRCT, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zCRCT").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zCRCT minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcrct(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zCRCT converted back to CRCT"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any CRCT to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzCRCT->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
