...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CRCT)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sCRCTPercentage, QString& szCRCTPercentage)
{
    int nPrecision = 2;
    ...
    szCRCTPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sCRCTPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount crctAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // CRCT Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = crctAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // CRCT labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, crctAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelCRCTPercent->setText(sPercentage);
    ui->labelzCRCTPercent->setText(szPercentage);
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in circuit.conf");
    }
    ...
    // CRCT Available
    bool showCRCTAvailable = settingShowAllBalances || crctAvailableBalance != nTotalBalance;
    bool showWatchOnlyCRCTAvailable = showCRCTAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showCRCTAvailable || showWatchOnlyCRCTAvailable);
    ui->labelBalance->setVisible(showCRCTAvailable || showWatchOnlyCRCTAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlyCRCTAvailable && showWatchOnly);

    // CRCT Pending
    bool showCRCTPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyCRCTPending = showCRCTPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showCRCTPending || showWatchOnlyCRCTPending);
    ui->labelUnconfirmed->setVisible(showCRCTPending || showWatchOnlyCRCTPending);
    ui->labelWatchPending->setVisible(showWatchOnlyCRCTPending && showWatchOnly);

    // CRCT Immature
    bool showCRCTImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showCRCTImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showCRCTImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showCRCTImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // CRCT Locked
    bool showCRCTLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyCRCTLocked = showCRCTLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showCRCTLocked || showWatchOnlyCRCTLocked);
    ui->labelLockedBalance->setVisible(showCRCTLocked || showWatchOnlyCRCTLocked);
