...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CRCT.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZcrctSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZcrct(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzCRCTReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCRCTReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCRCTReset.connect(boost::bind(NotifyzCRCTReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCRCTReset.disconnect(boost::bind(NotifyzCRCTReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcrctTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
