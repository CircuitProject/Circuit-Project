...
CzCRCTWallet::CzCRCTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcrct seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCRCTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCRCTSeed_deprecated()) {
                    LogPrintf("%s: Updated zCRCT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcrct seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcrct wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCRCTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCRCTCount(nCountLastUsed);
    else if (!walletdb.ReadZCRCTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCRCTWallet::Lock()
{
    seedMaster = 0;
}

void CzCRCTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCRCTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCRCT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCRCTWallet::LoadMintPoolFromDB()
{
...
void CzCRCTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCRCTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCRCTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcrctTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCRCTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCRCT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcrctTracker which also adds to database
    pwalletMain->zcrctTracker->Add(dMint, true);
...
void CzCRCTWallet::SeedToZCRCT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCRCTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCRCTWallet::GenerateDeterministicZCRCT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCRCTWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCRCTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCRCTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
