// Copyright (c) 2018-2019 The CIRCUIT developers
...
CzCRCTTracker::CzCRCTTracker(std::string strWalletFile)
{
...
CzCRCTTracker::~CzCRCTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCRCTTracker::Init()
{
...
bool CzCRCTTracker::Archive(CMintMeta& meta)
{
...
bool CzCRCTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCRCTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCRCTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCRCTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCRCTTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCRCTTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCRCTTracker::GetSerialHashes()
{
...
CAmount CzCRCTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCRCTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCRCTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCRCTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCRCTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCRCTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCRCTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCRCTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCRCTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCRCTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCRCTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCRCTWallet* zCRCTWallet)
{
    bool iszCRCTWalletInitialized = (NULL != zCRCTWallet);
    CMintMeta meta;
    ...
    if (! iszCRCTWalletInitialized)
        zCRCTWallet = new CzCRCTWallet(strWalletFile);
    meta.isSeedCorrect = zCRCTWallet->CheckSeed(dMint);
    if (! iszCRCTWalletInitialized)
        delete zCRCTWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCRCTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCRCTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCRCTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCRCTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCRCTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCRCTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCRCTWallet* zCRCTWallet = new CzCRCTWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCRCTWallet);
        }
        delete zCRCTWallet;
        LogPrint("zero", "%s: added %d dzcrct from DB\n", __func__, listDeterministicDB.size());
    }
