...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zCRCT v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCRCT spend tx %s already in block %d",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert CRCT to zCRCT
        ...
        if (isBlockFromFork) {
            ...
            if(hasZCRCTInputs){
                for (const CTxIn& zCrctInput : zCRCTInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zCrctInput);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCRCT mints and %d zCRCT spends\n", __func__, nMints, nSpends);
