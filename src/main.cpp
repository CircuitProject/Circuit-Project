...
#include "zcrct/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
/** Fees smaller than this (in ucrct) are considered zero fee (for relaying and mining)
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zCRCT spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcrct");
                }
                ...
                if (isPublicSpend) {
                    ...
                    if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                        return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                                __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zcrct");
                    }
            ...
            // Check that zCRCT mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight <= 527998) {
        ret = COIN * 1000;
    }
    else if (nHeight <= 790997) {
        ret = COIN * 800;
    }
    else if (nHeight <= 1053996) {
        ret = COIN * 600;
    }
    else if (nHeight <= 1842500) {
        ret = COIN * 500;
    }
    else if (nHeight <= 2631004) {
        ret = COIN * 250;
    }
    else if (nHeight <= 8724033) {
        ret = COIN * 200;
    }
    else {
        ret = COIN * 0;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, bool isZCRCTStake)
{
    int64_t ret = 0;
    ...
    if(nHeight < Params().LAST_POW_BLOCK()){
        ...
        ret = blockValue * 39 / 100;
    }
    else if(nHeight <= 527998){
        ret = blockValue * 49 / 100;
    }
    else if(nHeight <= 790997){
        ret = blockValue * 54 / 100;
    }
    else if(nHeight <= 1053996){
        ret = blockValue * 59 / 100;
    }
    else if(nHeight <= 1842500){
        ret = blockValue * 64 / 100;
    }
    else if(nHeight <= 2631004){
        ret = blockValue * 69 / 100;
    }
    else {
        ret = blockValue * 79 / 100;
    }

    return ret;
}

int64_t GetDevelopersPayment(int nHeight, int64_t blockValue, bool isZCRCTStake) {
    int64_t ret = 0;
...
void RecalculateZCRCTMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZCRCT..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZCRCT..."), percent);
        }
...
void RecalculateZCRCTSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZCRCT..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZCRCT..."), percent);
        }

        //Rewrite zCRCT supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zCRCT supply
...
bool RecalculateCRCTSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating CRCT supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating CRCT supply..."), percent);
        }
...
bool UpdateZCRCTSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
   ...
   // }
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zCRCT mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    //Track zCRCT money supply in the block index
    if (!UpdateZCRCTSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zCRCT supply for block=%s height=%d", __func__,
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 CRCT.
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zcrct", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zcrct", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
